...
public:
  Patrol()
      : Node("patrol_node"), direction_(0.0f), obstacle_detected_(false),
        state_(State::PATROLLING), current_yaw_(0.0f), target_yaw_(0.0f),
        start_yaw_(0.0f) {
    // Queue depth of 20 — absorbs message bursts over a remote network
    // connection
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "Patrol node started — tank mode, odom-guided, latency-tolerant.");
  }

private:
  // ── Utility
  // ────────────────────────────────────────────────────────────────── Derives
  // a scan array index from a real-world angle. Uses the scan's own metadata —
  // works on any lidar model or mounting.
  int angle_to_index(const sensor_msgs::msg::LaserScan::SharedPtr &msg,
                     float angle) const {
  ...
  void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    if (state_ == State::STOPPING || state_ == State::ROTATING)
      return;
  ...
  void control_loop() {
    ...
    case State::PATROLLING:
      ...
      if (obstacle_detected_) {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Obstacle detected — stopping. Target yaw: %.2f rad",
                    target_yaw_);
      } else {
        cmd.linear.x = LINEAR_VEL;
        cmd.angular.z = 0.0f;
    ...
    case State::ROTATING: {
      ...
      const float rotated_so_far =
          std::abs(normalize_angle(current_yaw_ - start_yaw_));

      if (rotated_so_far >= MIN_ROTATION &&
          std::abs(yaw_error) < YAW_TOLERANCE) {
        obstacle_detected_ = false;
        cooldown_start_ = Clock::now(); // start post-rotation immunity
        state_ = State::PATROLLING;
        RCLCPP_INFO(this->get_logger(),
                    "Rotation complete (%.2f rad) — resuming patrol.",
                    rotated_so_far);
      } else {
  ...
  static constexpr float OBSTACLE_THRESHOLD =
      0.5f;                                     // m     — detection distance
  static constexpr float ROTATION_SPEED = 0.3f; // rad/s — in-place turn speed
  static constexpr float YAW_TOLERANCE =
      0.1f; // rad   — rotation completion threshold (generous for network lag)
  static constexpr float MIN_ROTATION =
      0.5f; // rad   — minimum physical turn before completion check
  static constexpr float STOP_DURATION_S =
      0.5f; // s     — full stop duration before rotating
  static constexpr float COOLDOWN_DURATION_S =
      1.5f; // s     — post-rotation forward movement before re-arming
  ...
  Clock::time_point
      state_enter_time_; // wall-clock timestamp of last state transition
  Clock::time_point
      cooldown_start_; // wall-clock timestamp of post-rotation cooldown
